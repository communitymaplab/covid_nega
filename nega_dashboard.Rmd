...
output$map1 <- renderLeaflet({
  ...
  #select variable
  census.map <- select(census.vars, County, input$census_choice) 
  ...
  map1 <- leaflet(census.map) %>%
    addTiles() %>%
    addPolygons(fillOpacity = input$poly_alpha,
              fillColor = ~binpal(value),
              stroke = T,
              color = "black", opacity = 1, weight = 0.5) %>%
    addCircles(data = crop.points, color = "black", opacity = 1,
             popup = ~textbox, layerId = ~id) %>%
    addLegend("bottomright", pal = binpal, values = ~value,
...
```{r, eval = F}
#this was giving me lots of trouble, so I gave up
observeEvent(input$map1_circle_click,{
  serv_text <- services.all %>% filter(id==input$map1_circle_click$id)
